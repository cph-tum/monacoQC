...
classdef device < handle
    ...
    methods
        ...
        function eps = get.rel_permittivity(obj)
            ...
            for i = 1:length(ind_w)
                eps_mat(i) = (obj.layers{ind_w(i)}.material.eps_r * ...
        ...
        function cbo = get_CBO(obj)
            ...
            for i = 2:length(obj.layers)
                b(i) = obj.layers{i-1}.material.get_Ec(obj.substrate, ...
                    obj.orientation) - obj.layers{i}.material.get_Ec ...
                    (obj.substrate, obj.orientation);
            end
            b_count = gt(b, 0);
        ...
        function h2 = get_strain(obj, T, N)
            ...
            for i = 1:(length(obj.layers) - 1) / 5 * 2
                h = h + ...
                    ...
                    obj.substrate, obj.orientation) * obj.layers{1, i}.length;
                if (nargin < 3)
                    h_crit = h_crit + ...
                        (obj.layers{1, i}.material.a_lattice - asub) / asub * ...
                        obj.layers{1, i}.material.get_hcrit(obj.substrate, obj.orientation);
                else
                    h_crit = h_crit + ...
                        (obj.layers{1, i}.material.a_lattice - asub) / asub * ...
                        obj.layers{1, i}.material.get_hcrit(obj.substrate, obj.orientation, N);
                end
            end
            h2.h = h / 2;
        ...
        function new_x = strain_comp(obj, T, CBO_target)
            ...
            for i = 1:size(cell_mat, 2)
                if isa(cell_mat{1, i}, 'binary')
                    lb(i) = 0;
                    ub(i) = 0;
                    x0(i) = 0;
                elseif isa(cell_mat{1, i}, 'ternary')
                    lb(i) = 0;
                    ub(i) = 1;
                    x0(i) = cell_mat{1, i}.conc;
                else
                    error(['Material of type "%s"', ...
        ...
        % for device layers.
        function parab_xy = get_vec_parab(obj)
            % Generate vector with size of layers.
        ...
        function meff = get_vec_meff(obj)
            % Generate vector with size of layers.
        ...
        % for device layers.
        function vec_alpha = get_vec_alpha(obj)
            % Generate vector with size of layers.
    ...
    methods (Access = private)
        ...
        function res = f_strain(obj, T, x, CBO_target)
            ...
            for i = 1:size(mat_test, 2)
                mat_test{1, i}.conc = x(i);
                epsII(i) = mat_test{1, i}.get_eps_II(obj.substrate, obj.orientation);
                a(i) = mat_test{1, i}.a_lattice;
                G(i) = mat_test{1, i}.get_G(obj.substrate, obj.orientation);
                Ec_test(i) = mat_test{1, i}.get_Ec(obj.substrate, obj.orientation);
            end
